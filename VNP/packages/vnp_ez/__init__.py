...
def to_time_series(data: pd.DataFrame, time_feature, auto_sort=True, unit=None):
    data_copy = data.copy()
    data_copy[time_feature] = pd.to_datetime(data_copy[time_feature], unit=unit)
...
def auto_shift(data: pd.DataFrame, lag, features_to_shift: list, shifted_feature_name="*_prev_i", auto_drop_na=False,
               return_features=True, drop_base_shifted_features=False, target_feature=None):
    data_copy = data.copy()

    features_to_shift_complete = features_to_shift.copy()

    if target_feature is not None:
        features_to_shift_complete += [target_feature]

    for feature in features_to_shift_complete:
    ...
    if drop_base_shifted_features:
        data_copy = data_copy.drop(columns=features_to_shift)
...
def fill_data(data, freq="D"):
    data_copy = data.copy()
    full_dates = pd.date_range(start=data_copy.index.min(), end=data_copy.index.max(), freq=freq)
    data_copy = data_copy.reindex(full_dates)
    return data_copy


def detailed_describe(data: pd.DataFrame, features: list, outliers_method="iqr", show_visualization=True):
    descriptions = []
    data_copy = data.copy()
    if outliers_method == "iqr":
        for feature in features:
            data_description = data[feature].describe()
            Q1 = data_copy[feature].quantile(0.25)
            Q3 = data_copy[feature].quantile(0.75)
            IQR = Q3 - Q1
            lower_bound = Q1 - 1.5 * IQR
            upper_bound = Q3 + 1.5 * IQR
            lower_outliers = data_copy[(data_copy[feature] < lower_bound)][feature]
            upper_outliers = data_copy[data_copy[feature] > upper_bound][feature]
            total_outliers = upper_outliers.count() + lower_outliers.count()
            outliers_percentage = total_outliers / data_description["count"] * 100
            data_description.loc["iqr"] = IQR
            data_description.loc["lower_bound"] = lower_bound
            data_description.loc["upper_bound"] = upper_bound
            data_description.loc["lower_outliers"] = upper_outliers.count()
            data_description.loc["upper_outliers"] = lower_outliers.count()
            data_description.loc["outliers_total"] = total_outliers
            data_description.loc["outliers%"] = outliers_percentage
            descriptions.append(data_description)

            if show_visualization:
                sns.boxplot(data[feature])
                plt.show()

    return pd.concat(descriptions, axis=1)


def remove_outliers_all(data: pd.DataFrame, features: list):
    data_copy = data.copy()
    for feature in features:
        data_copy = remove_outliers(data_copy, feature)
    return data_copy


def remove_outliers(data: pd.DataFrame, feature: str):
    data_copy = data.copy()
    Q1 = data_copy[feature].quantile(0.25)
    Q3 = data_copy[feature].quantile(0.27)
    IQR = Q3 - Q1
    lower_bound = Q1 - 1.5 * IQR
    upper_bound = Q3 + 1.5 * IQR
    data_copy_filtered = data_copy[(data_copy[feature] > lower_bound) & (data_copy[feature] < upper_bound)]
    return data_copy_filtered


def clip_outliers(data: pd.DataFrame, features: list):
    data_copy = data.copy()
    for feature in features:
        Q1 = data_copy[feature].quantile(0.25)
        Q3 = data_copy[feature].quantile(0.27)
